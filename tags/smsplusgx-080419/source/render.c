...
#ifdef NGC
	linebuf = &internal_buffer[overscan ? 14:0];
#else
	linebuf = (bitmap.depth == 8) ? &bitmap.data[(vline * bitmap.pitch) + (overscan ? 14:0)] : &internal_buffer[option.overscan ? 14:0];
#endif
...
void remap_8_to_16(int line)
{
    ...
    /* horizontal borders (option.overscan mode 2) */
